...
dev1 已完成不接入界面的只读基座：核心层可以定位同名 NFO、poster 与 fanart，安全解析现有 NFO，并保留完整原始 XML 供后续往返更新。此阶段不会自动载入界面，也不会写入、移动或创建用户文件；日常使用行为仍与 v0.5.0 相同。下一步 dev2 才会把“本地 NFO”接入现有逐字段来源菜单。详细范围、安全原则和阶段计划见 [`ROADMAP-v0.6.0.md`](ROADMAP-v0.6.0.md)。
