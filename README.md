...
`v0.7.0-dev2` 已把自定义目标路径接入主界面。用户可以选择“影片所在位置”“来源位置的番号文件夹”或“自定义根目录的番号文件夹”，并独立决定是否把影片重命名为番号；界面会实时显示最终影片绝对路径，保存预览继续列出影片、NFO 与图片的每个目标。自定义根目录支持按钮选择或直接粘贴绝对路径，若根目录本身已是番号则不会重复嵌套。同卷本地路径已经开放安全执行；跨盘符和 UNC 网络路径会在界面与 Core 计划中明确阻止，等待 dev3 的复制、SHA-256 校验和失败恢复事务。
...
- v0.7.0-dev2 已开放三种目标位置；当前只执行影片所在盘符内的本地目标，跨盘符与 UNC 网络路径将在 dev3 开放。
