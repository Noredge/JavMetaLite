...
`v0.8.0-rc1` 冻结 v0.8 的功能范围，不再加入新行为。它汇总已经通过分阶段验收的启动影片载入、安全偏好持久化和最多 5 条最近目标根目录；危险的“直接保存并覆盖”仍不会跨启动保留，不可用根目录仍只会提示并阻止保存，程序不会自动创建目标根目录。

RC1 的精简成品闭环检查已经全部通过，包括启动入口、安全偏好与最近目录、不可用目标隔离、真实自定义目标保存、本地重新载入，以及影片与无变化 sidecar 的 SHA-256 检查。完整记录见 [`MANUAL-ACCEPTANCE-v0.8.0-rc1.md`](MANUAL-ACCEPTANCE-v0.8.0-rc1.md)。
