...
| 项目 | 状态 | 备注 |
| --- | --- | --- |
| R01 RC1 启动与冻结状态 | PASS | 版本标识与安全默认值正常。 |
| R02 载入 Jellyfin 本地资料 | PASS | 本地 NFO、poster 和 fanart 正确进入编辑器。 |
| R03 混合候选与取消预览 | PASS | 本地/在线候选组合正常，取消保持零写入。 |
| R04 保存与磁盘结果 | PASS | 最终字段、图片引用和目录结果正确。 |
| R05 Jellyfin 重扫 | PASS | Jellyfin 正确显示更新结果且没有重复条目。 |
| R06 再次载入闭环 | PASS | 保存结果正确回读，无变化 sidecar 保持不变。 |
| 影片 SHA-256 | YES | 往返保存前后影片内容一致。 |
| 未知 XML | YES | 自定义元素及属性保留。 |

RC1 最终集成验收全部通过。结合 dev4 的 B01–B07 与完整自动化门禁，`v0.6.0-rc1` 可以无功能改动晋升为 `v0.6.0` 稳定版。
