...
1本の動画を個別に整理する、Windows向けの軽量メタデータエディターです。動画を選択するか、動画ファイルまたは単一作品の品番フォルダーを1つドロップし、情報を検索して各項目を確認・編集した後、保存前にすべてのファイル変更をプレビューできます。JavMetaLiteはメディアライブラリ全体をスキャンせず、ユーザーの確認前に動画を書き換えたり移動したりしません。
...
- 別ドライブや UNC 保存先では、安全な一時コピーを使用します。完全な SHA-256 検証が既定で、自己責任でファイルサイズのみを確認する高速モードも明示的に選択できます。
...
- 別ボリュームへの転送では、既定で元ファイルを削除する前にファイルサイズと SHA-256 を検証します。高速モードは内容検証を省略しますが、一時領域、サイズ確認、競合防止、ロールバックは維持します。
