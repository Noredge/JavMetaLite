...
| 项目 | 状态 | 备注 |
| --- | --- | --- |
| R1 R18.dev 资料与预览 | PASS | 英文标题、日文原标题、图片预览与 Gallery 均正常。 |
| R2 切换 LibreDMM | PASS | 切换来源后预览与全部剧照正常。 |
| R3 日志证据 | PASS | 日志包含实际 content ID 与剧照数量。 |
| 影片 SHA-256 | YES | 整理和重命名前后完全一致。 |

RC2 定向复测全部通过。结合 RC1 已通过的其余九项，v0.4.0 验收范围现已全部通过。

### 已知来源数据问题

R18.dev 的 START-237 当前把导演返回为 `Yazawa, Ready To Receive`。软件按来源原样导入该字段；这不是解析错位，因此不加入针对单片的硬编码修正。用户仍可在保存前检查并手动编辑导演。若未来多个作品出现同类结构化数据错误，再考虑使用其他来源交叉校验。
